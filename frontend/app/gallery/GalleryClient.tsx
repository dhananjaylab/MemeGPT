...
async function fetchMemes(page: number, sort: SortMode): Promise<{ memes: GeneratedMeme[]; total: number }> {
  try {
    ...
    // apiClient.getMemes returns a MemeListResponse or array
    if (Array.isArray(data)) {
      return { memes: data, total: data.length };
    }
    return { 
      memes: data.memes || [], 
      total: data.total || (Array.isArray(data.memes) ? data.memes.length : 0) 
    };
  } catch (err) {
