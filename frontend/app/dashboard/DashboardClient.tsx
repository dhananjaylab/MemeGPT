...
async function fetchUserData(token?: string) {
  try {
    ...
    return { 
      user, 
      memes: Array.isArray(memesResponse) ? memesResponse : memesResponse.memes || [], 
      total: memesResponse.total || (Array.isArray(memesResponse) ? memesResponse.length : 0) 
    };
  } catch (err) {
...
export function DashboardClient({ userEmail, apiToken }: DashboardClientProps) {
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id);
    try {
      const res = await apiClient.deleteMeme(id, apiToken);
      if (res.success) {
      ...
      <section id="my-memes">
        ...
        {totalMemes > 40 && (
          <div className="mt-4 text-center">
            <Link href="/gallery" className="btn-ghost text-xs" title="View all">
              View all in gallery →
            </Link>
          </div>
        )}
