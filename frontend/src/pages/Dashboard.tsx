        ...
        <motion.section variants={staggerChild} className="glass-card border border-border rounded-xl p-4 md:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-3">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${plan === 'api' ? 'bg-acid/20 text-acid border border-acid/40' :
        ...
        <section id="overview">
          ...
          <div className="mt-4 glass-card p-4 border border-border/40">
            ...
            <div className="h-1.5 bg-background rounded-full overflow-hidden border border-border/50">
              <div
                className={`h-full rounded-full transition-all duration-700 ${pct >= 90 ? "bg-red-400" : pct >= 70 ? "bg-amber-400" : "bg-acid shadow-glow-md"
                  }`}
                style={{ width: `${pct}%` }}
              />
            </div>
...
function PlanCard({
  ...
  return (
    <div
      className={`card-dark p-6 flex flex-col gap-5 ${accent ? "border-acid/40" : ""
        }`}
    >
      {accent && (
