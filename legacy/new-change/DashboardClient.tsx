...
async function fetchUserData(email: string, token?: string) {
  ...
  const [userRes, memesRes] = await Promise.all([
    fetch(`${API}/api/auth/me`, { headers, cache: "no-store" }),
    fetch(`${API}/api/memes?page=1&limit=40&user=me`, { headers, cache: "no-store" }),
  ]);

  const user = userRes.ok ? await userRes.json() : null;
  const memesData = memesRes.ok ? await memesRes.json() : { memes: [], total: 0 };
  return { user, memes: memesData.memes, total: memesData.total };
}

export function DashboardClient({ userEmail, apiToken }: DashboardClientProps) {
  ...
  useEffect(() => {
    fetchUserData(userEmail, apiToken)
      .then(({ user, memes, total }) => {
        setUser(user);
        setMemes(memes);
        setTotalMemes(total);
      })
      ...
      <section id="overview">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Shares",     value: memes.reduce((a, m) => a + m.share_count, 0).toLocaleString() },
          ].map((s) => (
      ...
      <section id="my-memes">
        ...
        {memes.length === 0 ? (
          <div className="card-dark p-10 text-center">
            ...
            <Link href="/" className="btn-acid text-xs">
          ...
          {plan === "api" && user?.api_key ? (
            <>
              ...
              <p className="font-mono text-[10px] text-muted mt-3">
                Keep this secret. Rotate it anytime from{" "}
              ...
              <div className="mt-4 pt-4 border-t border-border">
                <Link href="/api-docs" className="btn-ghost text-xs gap-1.5">
