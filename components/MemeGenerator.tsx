...
import { generateMemes } from '@/lib/api';
...
export function MemeGenerator() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to generate memes. Please try again.');
    } finally {
      if (!jobId) {
  ...
  const pollJobStatus = useCallback(async (id: string) => {
    ...
    const poll = async () => {
      try {
        const response = await fetch(`/api/backend/jobs/${id}`);
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="Enter a topic, situation, or story... (e.g., 'When you realize it's Monday morning' or 'The struggle of debugging code at 3 AM')"
